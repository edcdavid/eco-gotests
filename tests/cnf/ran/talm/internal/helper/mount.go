...
func PrepareEnvWithSmallMountPoint(client *clients.Settings) (string, error) {
	...
	_, err := cluster.ExecCommandOnSNORetries(client, 3, fmt.Sprintf("sudo mkdir -p %s", tsparams.BackupPath))
	if err != nil {
		return "", err
	}

	_, err = cluster.ExecCommandOnSNORetries(client, 3, fmt.Sprintf("sudo mkdir -p %s", tsparams.RANTestPath))
	if err != nil {
		return "", err
	}

	// find the next available loopback device (OS takes care of creating a new one if needed)
	loopbackDevicePath, err := cluster.ExecCommandOnSNORetries(client, 3, "sudo losetup -f")
	if err != nil {
		return "", err
	}
	...
	_, err = cluster.ExecCommandOnSNORetries(
	...
	_, err = cluster.ExecCommandOnSNORetries(client, 3, fmt.Sprintf("sudo mkfs.xfs -f -q %s", loopbackDevicePath))
	if err != nil {
		return "", err
	}

	// mount the fs to backup dir
	_, err = cluster.ExecCommandOnSNORetries(client, 3, fmt.Sprintf("sudo mount %s %s",
...
func DiskFullEnvCleanup(client *clients.Settings, loopbackDevicePath string) error {
	...
	for len(output) == 0 {
		...
		output, err = cluster.ExecCommandOnSNORetries(
	...
	if safeToDeleteBackupDir {
		_, err = cluster.ExecCommandOnSNORetries(client, 3, fmt.Sprintf("sudo rm -rf %s", tsparams.BackupPath))
		if err != nil {
			return err
		}
	} else {
	...
	_, err = cluster.ExecCommandOnSNORetries(client, 3, fmt.Sprintf("sudo rm -rf %s", tsparams.RANTestPath))
...
func unmountLoopback(client *clients.Settings, loopbackDevicePath, findmntOutput string) (bool, error) {
	...
	for _, devicePath := range outputArr {
		...
		for deviceType == "" {
			var err error
			deviceType, err = cluster.ExecCommandOnSNORetries(client, 3, fmt.Sprintf("lsblk %s -o TYPE -n", devicePath))
		...
		if deviceType == "part" {
			...
			if loopbackDevicePath == devicePath {
				...
				_, err := cluster.ExecCommandOnSNORetries(client, 3, fmt.Sprintf("sudo umount --detach-loop %s",
					tsparams.BackupPath))
				if err != nil {
					return false, err
				}
